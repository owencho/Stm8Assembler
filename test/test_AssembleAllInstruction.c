...
* bitOperationCheck ,get2ndCompValue ,addition2ndCompValueWithLength,
* getMOVOperand, hashNValueReturn ,outputMachineCode, machineCodeLengthFinder
* MachineCodeAllocateOutput ,assembleLDWOperand , assembleBTJXOperand ,assembleLDLDFOperand
* assembleOneOperand, assembleNoOperand ,assembleTwoComplexOperand ,assembleSymbolComplexOperand
*/
...
void test_assembleInstruction_given_invalid_instruction_expect_fail(void) {
    ...
    Try{
        tokenizer = createTokenizer(" 123 sraw 1242354354756 ");
        // invalid instruction 123 is not inside the STM8 instruction set
...
void test_assembleInstruction_given_lalala_instruction_expect_fail(void) {
    ...
    Try{
        ...
        // invalid instruction lalala as it is not inside the STM8 instruction set
        configureTokenizer(tokenizer,TOKENIZER_DOLLAR_SIGN_HEX);
        mcode = assembleInstruction(tokenizer);
        TEST_FAIL_MESSAGE("Expecting exeception to be thrown.");
    } Catch(ex) {
...
void test_notNullCheck_given_NULL_expect_no_exception_thrown(void){
    ...
    ConversionData dataFlag;
    Try{
        tokenizer = createTokenizer("   "); // pass as it is a NULL token
        configureTokenizer(tokenizer,TOKENIZER_DOLLAR_SIGN_HEX);
        notNullCheck(tokenizer);
    } Catch(ex) {
        dumpErrorMessage(ex);
...
void test_addition2ndCompValueWithLength_given_neghex31_expect_hexD1(void) {
    ...
    Try{
        ...
        operand = getOperand(tokenizer,(1<<SHORT_OFF_OPERAND));
        TEST_ASSERT_EQUAL_UINT16(SHORT_OFF_OPERAND, operand->type);
        //expected return as SHORT_OFF_OPERAND
        TEST_ASSERT_EQUAL_HEX8(0xCF, operand->dataSize.ms);
        //expect return 0xCF value due to 2nd compliment
        token = (IntegerToken *)getToken(tokenizer);
        value = addition2ndCompValueWithLength(operand,2,token);
         // the 2 at middle is the length of machineCode
        // which is used from the value of machineCodeLengthFinder function
...
// this function will throw exception when
// value is NA which the value is invalid or NULL , so it will thrown exception
void test_addition2ndCompValueWithLength_given_NA_expect_exception(void) {
    ...
    Try{
        ...
        token = (IntegerToken *)getToken(tokenizer);
        value = addition2ndCompValueWithLength(operand,2,token);
        //expect exception thrown as the value is null
        TEST_FAIL_MESSAGE("Expecting error exeception to be thrown.");
    } Catch(ex) {
        dumpErrorMessage(ex);
...
void test_getMOVOpcode_given_shortmem_byte_expect_return_byte_operand(void) {
    ...
    Try{
        ...
        configureTokenizer(tokenizer,TOKENIZER_DOLLAR_SIGN_HEX);
        token = (IntegerToken *)getToken(tokenizer); //get token for MOV
        dst = getOperand(tokenizer,(1<<SHORT_MEM_OPERAND | 1<LONG_MEM_OPERAND)); //getOperand for dst $12
        TEST_ASSERT_EQUAL_UINT16(SHORT_MEM_OPERAND, dst->type); // dst expected as SHORT_MEM_OPERAND
        TEST_ASSERT_EQUAL_UINT16(0x12, dst->dataSize.ms); // expected 0x12 value on dst
        commarCheck(tokenizer);
        src = getOperand(tokenizer,( 1<<BYTE_OPERAND)); //getOperand for src #$12
        TEST_ASSERT_EQUAL_UINT16(BYTE_OPERAND, src->type);  // src expected as BYTE_OPERAND
        TEST_ASSERT_EQUAL_UINT16(0x12, src->dataSize.ms); // expected 0x12 value on src
...
void test_getMOVOpcode_given_shortmem_longmem_expect_return_longmem_operand(void) {
    ...
    ConversionData dataFlag;
    Try{
        tokenizer = createTokenizer("   mov $12,$1200 "); //note : mov dst, src
        configureTokenizer(tokenizer,TOKENIZER_DOLLAR_SIGN_HEX);
        token = (IntegerToken *)getToken(tokenizer); //get token for MOV
        dst = getOperand(tokenizer,1<<SHORT_MEM_OPERAND); //getOperand for dst $12
        TEST_ASSERT_EQUAL_UINT16(SHORT_MEM_OPERAND, dst->type); // dst expected as SHORT_MEM_OPERAND
        TEST_ASSERT_EQUAL_UINT16(0x12, dst->dataSize.ms); // expected 0x12 value on dst
        commarCheck(tokenizer);
        src = getOperand(tokenizer,1<<LONG_MEM_OPERAND); //getOperand for src $1200
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, src->type);  // src expected as LONG_MEM_OPERAND
        TEST_ASSERT_EQUAL_UINT16(0x12, src->dataSize.ms); // expected 0x1200 value on src
        TEST_ASSERT_EQUAL_UINT16(0x00, src->dataSize.ls);
        operand = getMOVOperand(tokenizer,dst , src );
        TEST_ASSERT_NOT_NULL(operand);
...
void test_getMOVOpcode_given_longmem_byte_expect_return_byte_operand(void) {
    ...
    ConversionData dataFlag;
    Try{
        tokenizer = createTokenizer("   mov $1200,#$12 "); //note : mov dst, src
        configureTokenizer(tokenizer,TOKENIZER_DOLLAR_SIGN_HEX);
        token = (IntegerToken *)getToken(tokenizer); //get token for MOV
        dst = getOperand(tokenizer,1<<LONG_MEM_OPERAND); //getOperand for dst $1200
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, dst->type); // dst expected as LONG_MEM_OPERAND
        TEST_ASSERT_EQUAL_UINT16(0x12, dst->dataSize.ms); // expected 0x1200 value on dst
        TEST_ASSERT_EQUAL_UINT16(0x00, dst->dataSize.ls);
        commarCheck(tokenizer);
        src = getOperand(tokenizer,1<<BYTE_OPERAND); //getOperand for src #$12
        TEST_ASSERT_EQUAL_UINT16(BYTE_OPERAND, src->type); // src expected as BYTE_OPERAND
        TEST_ASSERT_EQUAL_UINT16(0x12, src->dataSize.ms); // expected 0x12 value on src
...
void test_getMOVOpcode_given_longmem_shortmem_expect_return_longmem_operand(void) {
    ...
    ConversionData dataFlag;
    Try{
        tokenizer = createTokenizer("   mov $1200,$22 "); //note : mov dst, src
        configureTokenizer(tokenizer,TOKENIZER_DOLLAR_SIGN_HEX);
        token = (IntegerToken *)getToken(tokenizer); //get token for MOV
        dst = getOperand(tokenizer,1<<LONG_MEM_OPERAND); //getOperand for dst  $1200
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, dst->type);  // dst expected as LONG_MEM_OPERAND
        TEST_ASSERT_EQUAL_UINT16(0x12, dst->dataSize.ms); // expected 0x1200 value on dst
        TEST_ASSERT_EQUAL_UINT16(0x00, dst->dataSize.ls);
        commarCheck(tokenizer); //commar Check
        src = getOperand(tokenizer,1<<SHORT_MEM_OPERAND); //getOperand for src $22
        TEST_ASSERT_EQUAL_UINT16(SHORT_MEM_OPERAND, src->type); // src expected as SHORT_MEM_OPERAND
        TEST_ASSERT_EQUAL_UINT16(0x22, src->dataSize.ms); // expected 0x22 value on src
        operand = getMOVOperand(tokenizer,dst , src );
        TEST_ASSERT_NOT_NULL(operand);
        ...
        //will passed in to another function with movMs2ndOpValue on
        // machineCodeAllocateOutput function to be added into machineCode
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, operand->type);
        TEST_ASSERT_EQUAL_UINT16(NA, operand->dataSize.extCode);
        TEST_ASSERT_EQUAL_UINT16(NA, operand->dataSize.code);
...
void test_getMOVOpcode_given_longmem_longmem_expect_return_longmem_operand(void) {
    ...
    ConversionData dataFlag;
    Try{
        tokenizer = createTokenizer("   mov $2200,$3123 ");  //note : mov dst, src
        configureTokenizer(tokenizer,TOKENIZER_DOLLAR_SIGN_HEX);
        token = (IntegerToken *)getToken(tokenizer); //get token for MOV
        dst = getOperand(tokenizer,1<<LONG_MEM_OPERAND); //getOperand for dst $2200
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, dst->type); // dst expected as LONG_MEM_OPERAND
        TEST_ASSERT_EQUAL_UINT16(0x22, dst->dataSize.ms); // expected 0x2200 value on dst
        TEST_ASSERT_EQUAL_UINT16(0x00, dst->dataSize.ls);
        commarCheck(tokenizer); //commar Check
        src = getOperand(tokenizer,1<<LONG_MEM_OPERAND); //getOperand for src $3123
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, src->type); // src expected as LONG_MEM_OPERAND
        TEST_ASSERT_EQUAL_UINT16(0x31, src->dataSize.ms); // expected 0x3123 value on src
        TEST_ASSERT_EQUAL_UINT16(0x23, src->dataSize.ls);
        operand = getMOVOperand(tokenizer,dst , src );
        TEST_ASSERT_NOT_NULL(operand);
        //LONG_MEM_OPERAND expected as dst is LONG_MEM_OPERAND
        //and src is LONG_MEM_OPERAND
...
void test_getMOVOpcode_given_shortmem_shortmem_expect_return_shortmem_operand(void) {
    ...
    ConversionData dataFlag;
    Try{
        tokenizer = createTokenizer("   mov $12,$12 "); //note : mov dst, src
        configureTokenizer(tokenizer,TOKENIZER_DOLLAR_SIGN_HEX);
        token = (IntegerToken *)getToken(tokenizer);
        dst = getOperand(tokenizer,(1<<SHORT_MEM_OPERAND | 1<LONG_MEM_OPERAND)); //getOperand for dst
...
//this movMs2ndOpValue on MachineCodeAllocateOutput used when both operand is LONG_MEM_OPERAND
// and only for mov instruction which the value must be two msb value of the src value
// eg of instruction (mov dst,src)
// nvalue is used when hashNValueReturn function is used for special instruction eg BJTF ,BJTH ,BCPL ,BCCM
// nvalue and movMs2ndOpValue is put as NA if it is not applicable
...
// eg machineCodeAllocateOutput(tokenizer,dataFlag ,operand,nvalue,movMs2ndOpValue);
void test_MachineCodeAllocateOutput_given_mov_expect_pass(void) {
    ...
    Try{
        ...
        // eg of mov instruction (mov dst,src)
        configureTokenizer(tokenizer,TOKENIZER_DOLLAR_SIGN_HEX);
        token = (IntegerToken *)getToken(tokenizer);
        //first it tokenize the first $1310 check with getOperand
        dst = getOperand(tokenizer,(1<<LONG_MEM_OPERAND));
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, dst->type); //expected LONG_MEM_OPERAND on dst
        TEST_ASSERT_EQUAL_HEX8(0x13, dst->dataSize.ms); //expected 0x1310 on dst
        TEST_ASSERT_EQUAL_HEX8(0x10, dst->dataSize.ls);
        //second it find the data Flag check with operand get from dst
        dataFlag = getDataFlag(&movCodeInfo,tokenizer,dst); //expected LONG_MEM_OPERAND flag
        ...
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, src->type); //expected LONG_MEM_OPERAND on src
        TEST_ASSERT_EQUAL_HEX8(0x12, src->dataSize.ms);  //expected 0x1200 on src
        TEST_ASSERT_EQUAL_HEX8(0x00, src->dataSize.ls);
        // get mov operand special combination with this getMOVOperand
        operand = getMOVOperand(tokenizer,dst , src );
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, operand->type);
        TEST_ASSERT_EQUAL_HEX8(0x00, operand->dataSize.ms); //ms will be ls bit of the src value
...
void test_MachineCodeAllocateOutput_given_bset_longmem_hash_expect_pass(void) {
    ...
    Try{
        ...
        TEST_ASSERT_EQUAL_INT(1,bitOpCheck); // return 1 as nvalue for BSET is 2*n
        // getOperand on $1000
        operand= getOperand(tokenizer,(1<<LONG_MEM_OPERAND));
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, operand->type); //expected LONG_MEM_OPERAND $1000
        TEST_ASSERT_EQUAL_HEX8(0x10, operand->dataSize.ms);
        TEST_ASSERT_EQUAL_HEX8(0x00, operand->dataSize.ls);
        // getDataFlag on this LONG_MEM_OPERAND
        dataFlag = getDataFlag(&bsetCodeInfo,tokenizer,operand); //expected LONG_MEM_OPERAND dataFlag
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, dataFlag.type);
        //check commar between two operand
        commarCheck(tokenizer);
...
// this machineCodeAllocateOutput doesnt take in any nvalue and movMs2ndOpValue
// which it put all as NA which the function will know this is not for nvalue and mov operand
void test_MachineCodeAllocateOutput_given_jp_longmem_expect_pass(void) {
    ...
    Try{
        ...
        // getOperand on $1000
        operand= getOperand(tokenizer,(1<<LONG_MEM_OPERAND));
        TEST_ASSERT_EQUAL_UINT16(LONG_MEM_OPERAND, operand->type); //expected LONG_MEM_OPERAND
        TEST_ASSERT_EQUAL_HEX8(0x10, operand->dataSize.ms);        //value $1000
        TEST_ASSERT_EQUAL_HEX8(0x00, operand->dataSize.ls);
        // getDataFlag on this LONG_MEM_OPERAND
